...
function normalizeSuspenseChildren(vnode) {
    ...
    if (shapeFlag & 32 /* SLOTS_CHILDREN */) {
        ...
        return {
            content: normalizeVNode(shared.isFunction(d) ? d() : d),
            fallback: normalizeVNode(shared.isFunction(fallback) ? fallback() : fallback)
        };
    }
    else {
        return {
...
function resolveAsset(type, name, warnMissing = true) {
    ...
    if (instance) {
        ...
        let res = registry[name] ||
            registry[(camelized = shared.camelize(name))] ||
            registry[(capitalized = shared.capitalize(camelized))];
...
function _createVNode(type, props = null, children = null, patchFlag = 0, dynamicProps = null, isBlockNode = false) {
    ...
    if (shared.isFunction(type) && '__vccOpts' in type) {
        type = type.__vccOpts;
    }
...
function emit(instance, event, ...args) {
    const props = instance.vnode.props || shared.EMPTY_OBJ;
    {
        ...
        if (options) {
            if (!(event in options)) {
                ...
                if (!propsOptions || !(`on` + shared.capitalize(event) in propsOptions)) {
                    warn(`Component emitted event "${event}" but it is neither declared in ` +
                        `the emits option nor as an "on${shared.capitalize(event)}" prop.`);
                }
            }
            else {
    ...
    let handler = props[`on${shared.capitalize(event)}`];
    // for v-model update:xxx events, also trigger kebab-case equivalent
    // for props passed via kebab-case
    if (!handler && event.startsWith('update:')) {
        event = shared.hyphenate(event);
        handler = props[`on${shared.capitalize(event)}`];
    }
    if (handler) {
...
function normalizeEmitsOptions(options) {
    ...
    else if (shared.isArray(options)) {
        if (options._n) {
            return options._n;
        }
        const normalized = {};
        options.forEach(key => (normalized[key] = null));
        shared.def(options, '_n', normalized);
        return normalized;
    }
    else {
        return options;
...
function isEmitListener(emits, key) {
    return (shared.isOn(key) &&
        (shared.hasOwn((emits = normalizeEmitsOptions(emits)), key[2].toLowerCase() + key.slice(3)) ||
            shared.hasOwn(emits, key.slice(2))));
}
            ...
            if (!rawProps ||
                ...
                if (options) {
                    if (rawPrevProps &&
                        ...
                        props[key] = resolvePropValue(options, rawProps || shared.EMPTY_OBJ, key, undefined);
                    }
                }
                else {
                    delete props[key];
...
function resolvePropValue(options, props, key, value) {
    ...
    if (opt != null) {
        ...
        if (hasDefault && value === undefined) {
            ...
            value =
                opt.type !== Function && shared.isFunction(defaultValue)
                    ? defaultValue()
                    : defaultValue;
        }
        // boolean casting
...
function assertType(value, type) {
    ...
    if (isSimpleType(expectedType)) {
        ...
        valid = shared.toRawType(value) === 'Object';
    }
...
function injectHook(type, hook, target = currentInstance, prepend = false) {
    if (target) {
        ...
        const apiName = `on${shared.capitalize(ErrorTypeStrings[type].replace(/ hook$/, ''))}`;
...
function injectToKeepAliveRoot(hook, type, target, keepAliveRoot) {
    ...
    onUnmounted(() => {
        shared.remove(keepAliveRoot[type], hook);
    }, target);
}
    ...
    const setupRenderEffect = (instance, initialVNode, container, anchor, parentSuspense, isSVG, optimized) => {
        ...
        instance.update = reactivity.effect(function componentEffect() {
            if (!instance.isMounted) {
                ...
                if (instance.refs !== shared.EMPTY_OBJ) {
                    instance.refs = {};
                }
                {
                    startMeasure(instance, `patch`);
...
function doWatch(source, cb, { immediate, deep, flush, onTrack, onTrigger } = shared.EMPTY_OBJ, instance = currentInstance) {
    ...
    const applyCb = cb
        ? () => {
            ...
            if (deep || shared.hasChanged(newValue, oldValue)) {
...
function applyOptions(instance, options, deferredData = [], deferredWatch = [], asMixin = false) {
    ...
    // asset options
    if (components) {
        shared.extend(instance.components, components);
    }
    if (directives) {
        shared.extend(instance.directives, directives);
    }
    // lifecycle options
    if (!asMixin) {
...
const PublicInstanceProxyHandlers = {
    get({ _: instance }, key) {
        ...
        ((globalProperties = appContext.config.globalProperties),
            ...
            key.indexOf('__v') !== 0) {
            if (data !== shared.EMPTY_OBJ && key[0] === '$' && shared.hasOwn(data, key)) {
...
function handleSetupResult(instance, setupResult, isSSR) {
    if (shared.isFunction(setupResult)) {
        ...
        instance.setupState = reactivity.reactive(setupResult);
        {
...
function defineComponent(options) {
    return shared.isFunction(options) ? { setup: options } : options;
}
...
function toHandlers(obj) {
    ...
    for (const key in obj) {
        ret[`on${shared.capitalize(key)}`] = obj[key];
    }
    return ret;
}
 ...
 * @private
 */
const _toDisplayString = shared.toDisplayString;
/**
 * @private
 */
const _camelize = shared.camelize;
const _ssrUtils = {
